...
func _snap_up_stairs_check(delta) -> bool:
	...
	and (down_check_result.get_collider().is_class("StaticBody3D") or down_check_result.get_collider().is_class("CSGShape3D"))):
		...
		stairs_ahead_ray_cast.global_position = down_check_result.get_position() + Vector3(0,MAX_STEP_HEIGHT,0) + expected_move_motion.normalized() * 0.1
		stairs_ahead_ray_cast.force_raycast_update()
		if not is_surface_too_steep(stairs_ahead_ray_cast.get_collision_normal()):
...
func add_points(point_val: int):
	var temp_points = int(points.text) + point_val
	points.text = str(temp_points)
